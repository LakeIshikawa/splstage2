...
{
	{ 0.2f, 0.2f, 0.2f, 0.2f, 0.2f, 0.2f, 0.2f, 0.2f, 0.2f, 0.2f, 0.2f, 0.2f },	// WIN
	{ 0.2f, 0.2f, 0.2f, 0.2f, 0.2f, 0.2f },										// WINBACK
...
int Majo::sRingoX[5] = { 179, 356, 531, 64, 616 };
...
void Majo::CollisionResponse(ICollidable *rCollObject, int rThisGroupId, int rOpGroupId)
{
	...
	if(jiki){ //当たり！
		if( rOpGroupId == SP->GRID_KOUGEKI && rThisGroupId == SP->GRID_BOGYO ){
			...
			//ﾋﾛｲﾝにﾀﾞﾒｰｼﾞ
			if( mStatus != WINDAMAGE && mStatus != DOORDAMAGE )
...
void Majo::Die()
{
	...
	if( mStatus != DIYING ){
		...
		// りんごを消去させる
		GAMECONTROL->GetMobManager()->RemoveAll(typeid(ThrowApple).name());
		GAMECONTROL->GetMobManager()->RemoveAll(typeid(RollingApple).name());
